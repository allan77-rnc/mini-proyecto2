...
| `webrtc:offer` | `{ targetSocketId, sdp, idToken }` | Relay SDP offer to a specific peer |
...
|---|---|---|
| `room:joined` | `{ roomId, socketId, participants[] }` | Confirms join; includes own socketId and current participants list |
| `room:user-joined` | `{ socketId, username }` | Notifies others when a user joins; includes socketId to initiate offer |
| `room:user-left` | `{ socketId, username }` | Notifies others when a user leaves; includes socketId to tear down RTCPeerConnection |
